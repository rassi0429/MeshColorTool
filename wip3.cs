...
namespace VRChatAvatarTools
{
    public class MeshColorEditorWindow : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            // Load ComputeShader
            textureProcessorCompute = AssetDatabase.LoadAssetAtPath<ComputeShader>("Assets/kokoa/MeshColorTextureProcessor.compute");
            if (textureProcessorCompute == null)
            {
                Debug.LogWarning("[MeshColorTool] ComputeShader not found at Assets/kokoa/MeshColorTextureProcessor.compute. Falling back to CPU processing.");
                useComputeShader = false;
            }
            else
            {
                Debug.Log("[MeshColorTool] ComputeShader loaded successfully. GPU acceleration enabled.");
            }
        }
        ...
        private void DrawActions()
        {
            ...
            // ComputeShader toggle
            EditorGUILayout.BeginHorizontal();
            EditorGUI.BeginChangeCheck();
            useComputeShader = EditorGUILayout.Toggle(GetLocalizedText("useComputeShader"), useComputeShader);
            if (EditorGUI.EndChangeCheck())
            {
                if (useComputeShader && textureProcessorCompute == null)
                {
                    textureProcessorCompute = AssetDatabase.LoadAssetAtPath<ComputeShader>("Assets/kokoa/MeshColorTextureProcessor.compute");
                    if (textureProcessorCompute == null)
                    {
                        Debug.LogWarning("[MeshColorTool] ComputeShader not found. GPU acceleration disabled.");
                        useComputeShader = false;
                    }
                }
            }
            
            if (textureProcessorCompute != null && SystemInfo.supportsComputeShaders)
            {
                EditorGUILayout.LabelField("GPU", GUILayout.Width(30));
            }
            else
            {
                EditorGUILayout.LabelField("CPU", GUILayout.Width(30));
            }
            EditorGUILayout.EndHorizontal();
        ...
        private void OnSceneGUI(SceneView sceneView)
        {
            ...
            {
                Debug.Log("[MESH COLOR TIMER] === CLICK DETECTED - STARTING PERFORMANCE ANALYSIS ===");
                ...
                debugInfo += "\n=== CLICK EVENT PERFORMANCE ANALYSIS ===\n";
                ...
                stepTimer.Stop();
                var initialSetupTime = stepTimer.ElapsedMilliseconds;
                debugInfo += $"[TIMER] Initial setup: {initialSetupTime}ms\n";
                Debug.Log($"[MESH COLOR TIMER] Initial setup: {initialSetupTime}ms");
                stepTimer.Restart();
                ...
                if (tempCollider != null && targetMeshRenderer != null)
                {
                    ...
                    stepTimer.Stop();
                    debugInfo += $"[TIMER] Mesh creation: {stepTimer.ElapsedMilliseconds}ms\n";
                    stepTimer.Restart();
                    
                    targetMeshRenderer.BakeMesh(bakedMesh);
                    stepTimer.Stop();
                    var bakeMeshTime = stepTimer.ElapsedMilliseconds;
                    debugInfo += $"[TIMER] BakeMesh(): {bakeMeshTime}ms\n";
                    Debug.Log($"[MESH COLOR TIMER] BakeMesh(): {bakeMeshTime}ms");
                    stepTimer.Restart();
                    
                    tempCollider.sharedMesh = bakedMesh;
                    stepTimer.Stop();
                    debugInfo += $"[TIMER] Assign to collider: {stepTimer.ElapsedMilliseconds}ms\n";
                    stepTimer.Restart();
                ...
                stepTimer.Stop();
                debugInfo += $"[TIMER] Pre-raycast setup: {stepTimer.ElapsedMilliseconds}ms\n";
                stepTimer.Restart();
                
                if (Physics.Raycast(ray, out hit))
                {
                    stepTimer.Stop();
                    var raycastTime = stepTimer.ElapsedMilliseconds;
                    debugInfo += $"[TIMER] Physics.Raycast (HIT): {raycastTime}ms\n";
                    Debug.Log($"[MESH COLOR TIMER] Physics.Raycast (HIT): {raycastTime}ms");
                    stepTimer.Restart();
                    ...
                    if (hit.collider == tempCollider)
                    {
                        ...
                        SelectMeshArea(hit.point, totalStartTime, stepTimer);
                ...
                {
                    stepTimer.Stop();
                    debugInfo += $"[TIMER] Physics.Raycast (MISS): {stepTimer.ElapsedMilliseconds}ms\n";
                    stepTimer.Restart();
                    ...
                    {
                        stepTimer.Stop();
                        debugInfo += $"[TIMER] Long distance raycast: {stepTimer.ElapsedMilliseconds}ms\n";
                        stepTimer.Restart();
                ...
                stepTimer.Stop();
                debugInfo += $"[TIMER] Final cleanup: {stepTimer.ElapsedMilliseconds}ms\n";
                
                totalStartTime.Stop();
                var totalClickTime = totalStartTime.ElapsedMilliseconds;
                debugInfo += $"[TIMER] *** TOTAL CLICK EVENT TIME: {totalClickTime}ms ***\n";
                Debug.Log($"[MESH COLOR TIMER] *** TOTAL CLICK EVENT TIME: {totalClickTime}ms ***");
                debugInfo += "=== END CLICK EVENT PERFORMANCE ANALYSIS ===\n";
        ...
        private void SelectMeshArea(Vector3 hitPoint, System.Diagnostics.Stopwatch totalTimer = null, System.Diagnostics.Stopwatch stepTimer = null)
        {
            ...
            {
                stepTimer.Stop();
                debugInfo += $"[TIMER] Enter SelectMeshArea: {stepTimer.ElapsedMilliseconds}ms\n";
                stepTimer.Restart();
            }
            
            debugInfo += "\n--- SelectMeshArea PERFORMANCE ANALYSIS ---\n";
            ...
            {
                stepTimer.Stop();
                debugInfo += $"[TIMER] Setup variables: {stepTimer.ElapsedMilliseconds}ms\n";
                stepTimer.Restart();
            }
            ...
            {
                stepTimer.Stop();
                var vertexCalcTime = stepTimer.ElapsedMilliseconds;
                debugInfo += $"[TIMER] Vertex distance calculation (All {vertices.Length} vertices): {vertexCalcTime}ms\n";
                Debug.Log($"[MESH COLOR TIMER] Vertex distance calculation ({vertices.Length} vertices): {vertexCalcTime}ms");
                stepTimer.Restart();
            }
            ...
            {
                stepTimer.Stop();
                debugInfo += $"[TIMER] Sort candidates: {stepTimer.ElapsedMilliseconds}ms\n";
                stepTimer.Restart();
            }
            ...
            {
                stepTimer.Stop();
                debugInfo += $"[TIMER] Find closest vertices: {stepTimer.ElapsedMilliseconds}ms\n";
                stepTimer.Restart();
            }
            ...
            {
                stepTimer.Stop();
                debugInfo += $"[TIMER] Normal calculation and vertex selection: {stepTimer.ElapsedMilliseconds}ms\n";
                stepTimer.Restart();
            }
            ...
            if (selectedVertex >= 0)
            {
                ...
                {
                    stepTimer.Stop();
                    debugInfo += $"[TIMER] Flood fill setup: {stepTimer.ElapsedMilliseconds}ms\n";
                    stepTimer.Restart();
                }
                ...
                {
                    stepTimer.Stop();
                    var floodFillTime = stepTimer.ElapsedMilliseconds;
                    debugInfo += $"[TIMER] *** FLOOD FILL ALGORITHM: {floodFillTime}ms ***\n";
                    Debug.Log($"[MESH COLOR TIMER] *** FLOOD FILL ALGORITHM: {floodFillTime}ms ***");
                    stepTimer.Restart();
                }
                ...
                {
                    stepTimer.Stop();
                    debugInfo += $"[TIMER] Selection management: {stepTimer.ElapsedMilliseconds}ms\n";
                    stepTimer.Restart();
                }
                ...
                if (showPreview)
                {
                    ...
                    {
                        stepTimer.Stop();
                        debugInfo += $"[TIMER] Set preview update flag: {stepTimer.ElapsedMilliseconds}ms\n";
                    }
                }
                
                if (stepTimer != null && totalTimer != null)
                {
                    totalTimer.Stop();
                    var totalSelectTime = totalTimer.ElapsedMilliseconds;
                    debugInfo += $"[TIMER] *** TOTAL SelectMeshArea TIME: {totalSelectTime}ms ***\n";
                    Debug.Log($"[MESH COLOR TIMER] *** TOTAL SelectMeshArea TIME: {totalSelectTime}ms ***");
                }
                
                debugInfo += "--- END SelectMeshArea PERFORMANCE ANALYSIS ---\n";
        ...
        private Texture2D CreateModifiedTextureWithAllSelections()
        {
            ...
            {
                Debug.Log("[MESH COLOR TIMER] Using ComputeShader for texture processing");
                return CreateModifiedTextureWithComputeShader();
            }
            else
            {
                Debug.Log("[MESH COLOR TIMER] Using CPU for texture processing");
                return CreateModifiedTextureWithCPU();
            }
        }
        ...
        private Texture2D CreateModifiedTextureWithComputeShader()
        {
            ...
            stepTimer.Stop();
            Debug.Log($"[MESH COLOR TIMER] ComputeShader - Source texture prep: {stepTimer.ElapsedMilliseconds}ms");
            stepTimer.Restart();
            ...
            // Build triangle UV data and selection enabled flags
            ...
            stepTimer.Stop();
            Debug.Log($"[MESH COLOR TIMER] ComputeShader - Data preparation: {stepTimer.ElapsedMilliseconds}ms");
            stepTimer.Restart();
            ...
            stepTimer.Stop();
            Debug.Log($"[MESH COLOR TIMER] ComputeShader - Buffer setup: {stepTimer.ElapsedMilliseconds}ms");
            stepTimer.Restart();
        ...
        private Texture2D CreateModifiedTextureWithCPU()
        {
        ...
        {
            // Load compute shader
            ComputeShader paintTriangleCompute = AssetDatabase.LoadAssetAtPath<ComputeShader>("Assets/kokoa/Editor/PaintTriangle.compute");
            if (paintTriangleCompute == null)
            {
                Debug.LogError("PaintTriangle.compute not found! Falling back to CPU implementation.");
                PaintTriangleOnMaskCPU(texture, uv0, uv1, uv2, paintedPixels);
                return;
            }

            int kernelHandle = paintTriangleCompute.FindKernel("PaintTriangle");
            ...
            // Create render texture for compute shader
            RenderTexture resultTexture = new RenderTexture(texture.width, texture.height, 0, RenderTextureFormat.ARGB32);
            resultTexture.enableRandomWrite = true;
            resultTexture.Create();
            
            // Copy original texture to render texture
            Graphics.Blit(texture, resultTexture);
            
            // Create buffers for painted pixels tracking
            int maxPixels = 65536;
            ComputeBuffer paintedPixelsBuffer = new ComputeBuffer(maxPixels, sizeof(uint) * 2);
            ComputeBuffer pixelCounterBuffer = new ComputeBuffer(1, sizeof(uint));
            
            // Initialize counter buffer
            uint[] counterInit = { 0 };
            pixelCounterBuffer.SetData(counterInit);
            
            // Set compute shader parameters
            paintTriangleCompute.SetTexture(kernelHandle, "ResultTexture", resultTexture);
            paintTriangleCompute.SetBuffer(kernelHandle, "PaintedPixels", paintedPixelsBuffer);
            paintTriangleCompute.SetBuffer(kernelHandle, "PixelCounter", pixelCounterBuffer);
            
            paintTriangleCompute.SetInts("vertex0", x0, y0, 0);
            paintTriangleCompute.SetInts("vertex1", x1, y1, 0);
            paintTriangleCompute.SetInts("vertex2", x2, y2, 0);
            paintTriangleCompute.SetInts("textureSize", texture.width, texture.height);
            paintTriangleCompute.SetInts("boundingBox", minX, maxX, minY, maxY);
            
            // Calculate thread groups
            int threadGroupsX = Mathf.CeilToInt((maxX - minX + 1) / 8.0f);
            int threadGroupsY = Mathf.CeilToInt((maxY - minY + 1) / 8.0f);
            
            // Dispatch compute shader
            paintTriangleCompute.Dispatch(kernelHandle, threadGroupsX, threadGroupsY, 1);
            ...
            // Get painted pixels from compute buffer
            uint[] pixelCountData = new uint[1];
            pixelCounterBuffer.GetData(pixelCountData);
            uint paintedCount = pixelCountData[0];
            
            if (paintedCount > 0)
            {
                uint[] paintedPixelsData = new uint[maxPixels * 2];
                paintedPixelsBuffer.GetData(paintedPixelsData);
                
                for (int i = 0; i < Mathf.Min(paintedCount, maxPixels); i++)
                {
                    int x = (int)paintedPixelsData[i * 2];
                    int y = (int)paintedPixelsData[i * 2 + 1];
            ...
            paintedPixelsBuffer.Release();
            pixelCounterBuffer.Release();
        }
        
        // Fallback CPU implementation
        private void PaintTriangleOnMaskCPU(Texture2D texture, Vector2 uv0, Vector2 uv1, Vector2 uv2, HashSet<Vector2Int> paintedPixels)
        {
            int x0 = Mathf.RoundToInt(uv0.x * (texture.width - 1));
            int y0 = Mathf.RoundToInt(uv0.y * (texture.height - 1));
            int x1 = Mathf.RoundToInt(uv1.x * (texture.width - 1));
        ...
        private void UpdatePreview()
        {
            ...
            stepTimer.Stop();
            debugInfo += $"[TIMER] Apply preview material: {stepTimer.ElapsedMilliseconds}ms\n";
            
            previewTimer.Stop();
            var totalPreviewTime = previewTimer.ElapsedMilliseconds;
        ...
        private string GetLocalizedText(string key)
        {
            if (currentLanguage == Language.Japanese)
            {
                switch (key)
                {
                    ...
                    case "useComputeShader": return "GPU加速を使用";
            ...
            else // English
            {
                switch (key)
                {
                    ...
                    case "useComputeShader": return "Use GPU Acceleration";
